...
import { AllCommunityModule, ModuleRegistry, ColDef, CellValueChangedEvent, SelectionChangedEvent, GridReadyEvent } from 'ag-grid-community';
...
export default function DataTable({ records, onRecordsChange, onSelectionChange }: DataTableProps) {
  ...
  const onCellValueChanged = useCallback((event: CellValueChangedEvent) => {
    ...
    if (field === 'intervalFrom' || field === 'intervalUpTo' || field === 'overlapClaimedDate') {
      const updatedRecords = recalculateAll(records.map((r) =>
      ...
      const updatedRecords = records.map((r) =>
        r.id === event.data.id ? { ...r, [field as string]: event.newValue } : r
      );
      onRecordsChange(updatedRecords);
    }
